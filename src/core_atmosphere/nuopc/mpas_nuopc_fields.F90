...
module mpas_nuopc_fields
  use mpas_nuopc_utils, only: create_esmf_mesh
  use esmf
  use nuopc
...
! #endif
  ...
  subroutine field_init()
    ! type(logical), intent(in), optional :: noahmp
    field_list = [ &
      add_field("inst_total_soil_moisture_content","smc", "m3 m-3", &
        IMPORT_F, EXPORT_F, 0.20d0), &
    ...
    ! if (present(noahmp)) then
    !    if (noahmp .eqv. .true.) then
  ...
  ! did, realizeAllImport, realizeAllExport, memr_import, memr_export, rc)
    ...
    if (.not. mesh_esmf_initialized) then
       ...
       mesh_esmf = create_esmf_mesh(domain, rc=rc)
       ESMF_ERR_RETURN(rc)
